...
def mock_engine():
    ...
    with engine.connect() as conn:
        conn.execute(text("CREATE TABLE test (id INTEGER, churn_risk FLOAT)"))
    return engine


@patch('src.sql_translator.llm')  # Mock LLM to avoid real calls
def test_translate_to_sql(mock_llm, mock_engine):
    mock_llm.invoke.return_value = AIMessage(content="SELECT * FROM test WHERE churn_risk > 0.8")
...
@patch('src.sql_translator.llm')  # Mock LLM to return invalid SQL
def test_invalid_sql(mock_llm, mock_engine):
    mock_llm.invoke.return_value = AIMessage(content="INVALID SQL HERE")  # Simulate invalid SQL
